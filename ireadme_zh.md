...
* 支持 HTTP 到 HTTP 代理功能（通过 `/http://` 前缀）
* 始终启用发送零拷贝（Zero Copy on Send）
* 无需轮询即可向客户端发送数据
* 无需分析 MPEG2-TS 流，智能地向新客户端发送 MPEG2-TS 头部

### HTTP 代理功能
新版本集成了 HTTP 到 HTTP 的代理功能，可以通过以下方式使用：

```
http://服务器地址:端口/http://目标网站地址
```

例如：
```
http://192.168.1.100:8080/http://example.com
```

该功能支持 IPv4 和 IPv6 地址，可以代理 HTTP 请求到目标服务器。
